  ...
  methods: {
    applyFilters() {
      ...
      switch (this.agefilter) {
        case "1to2":
          filteredData = filteredData.filter(
            (toy) => toy.age >= 0 && toy.age <= 2
          );
          break;
        case "3to5":
          filteredData = filteredData.filter(
            (toy) => toy.age >= 3 && toy.age <= 5
          );
          break;
        case "6to10":
          filteredData = filteredData.filter(
            (toy) => toy.age >= 6 && toy.age <= 9
          );
          break;
        case "10more":
      ...
      switch (this.pricefilter) {
        case "1to30":
          filteredData = filteredData.filter(
            (toy) => toy.price >= 0 && toy.price <= 30
          );
          break;
        case "31to60":
          filteredData = filteredData.filter(
            (toy) => toy.price >= 31 && toy.price <= 60
          );
          break;
        case "61to90":
          filteredData = filteredData.filter(
            (toy) => toy.price >= 61 && toy.price <= 90
          );
          break;
        case "100more":
